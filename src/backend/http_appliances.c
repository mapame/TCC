...
unsigned int http_handler_get_appliance_list(struct MHD_Connection *conn,
	...
	while((result = sqlite3_step(ppstmt)) == SQLITE_ROW) {
		...
		json_object_object_add_ex(json_appliance_item, "signature_qty", json_object_new_int(sqlite3_column_int(ppstmt, 8)), JSON_C_OBJECT_ADD_KEY_IS_NEW);
	}
...
unsigned int http_handler_get_appliance(struct MHD_Connection *conn,
	...
	while((result = sqlite3_step(ppstmt)) == SQLITE_ROW) {
		...
		json_object_object_add_ex(json_response, "signature_qty", json_object_new_int(sqlite3_column_int(ppstmt, 7)), JSON_C_OBJECT_ADD_KEY_IS_NEW);
	}
...
unsigned int http_handler_add_appliance_signatures(struct MHD_Connection *conn,
	...
	struct json_object *json_timestamp, *json_delta_pt, *json_peak_pt, *json_delta_p, *json_delta_s, *json_delta_q, *json_duration;
