...
int energy_add_power(power_data_t *pd) {
	...
	const char sql_store_minute[] = "INSERT INTO energy_minutes(timestamp,latest_second,active,reactive,min_p) VALUES(?1,?2,?3,?4,?5)"
									" ON CONFLICT(timestamp) DO UPDATE SET second_count = second_count + 1, latest_second = excluded.latest_second, active = active + excluded.active, reactive = reactive + excluded.reactive, min_p = min(min_p, excluded.min_p) WHERE latest_second < excluded.latest_second;";
	const char sql_store_hour[] = "INSERT INTO energy_hours(year,month,day,hour,active,reactive,min_p) VALUES(?1,?2,?3,?4,?5,?6,?7)"
									" ON CONFLICT(year,month,day,hour) DO UPDATE SET second_count = second_count + 1, active = active + excluded.active, reactive = reactive + excluded.reactive, min_p = min(min_p, excluded.min_p);";
	const char sql_store_day[] = "INSERT INTO energy_days(year,month,day,active,reactive,min_p) VALUES(?1,?2,?3,?4,?5,?6)"
									" ON CONFLICT(year,month,day) DO UPDATE SET second_count = second_count + 1, active = active + excluded.active, reactive = reactive + excluded.reactive, min_p = min(min_p, excluded.min_p);";
	const char sql_store_month[] = "INSERT INTO energy_months(year,month,active,reactive,min_p) VALUES(?1,?2,?3,?4,?5)"
									" ON CONFLICT(year,month) DO UPDATE SET second_count = second_count + 1, active = active + excluded.active, reactive = reactive + excluded.reactive, min_p = min(min_p, excluded.min_p);";
