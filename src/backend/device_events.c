...
int store_device_event_db(time_t timestamp, const char *description) {
	...
	const char sql_store_event[] = "INSERT INTO device_events(timestamp,description) VALUES(?1,?2);";
	...
	if((result = sqlite3_prepare_v2(db_conn, sql_store_event, -1, &ppstmt, NULL)) != SQLITE_OK) {
		LOG_ERROR("Failed to prepare the SQL statement for event storage: %s", sqlite3_errstr(result));
		sqlite3_close(db_conn);
